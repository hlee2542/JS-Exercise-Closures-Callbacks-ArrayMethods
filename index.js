...
function processSum(numberList, callback) {
  return callback(numberList.reduce((x, k) => x + k, 0));
}
...
function processContains(item, list, callback) {
  let isIn = false;
  let k = 0;
  while (!isIn && k < list.length) {
    if (list[k] === item) {
      isIn = true;
    }
    k++;
  }
  return callback(isIn);
}
...
function getFullNames(runners) {
  let lf = [];
  runners.forEach((runner) => lf.push(`${runner.last_name}, ${runner.first_name}`));
  return lf;
}
